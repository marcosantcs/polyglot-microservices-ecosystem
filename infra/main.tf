...
# ─── ECS TASK + SERVICE: order-service ──────────────────────────────
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "ConnectionStrings__DefaultConnection",
        value = "Host=${aws_db_instance.postgres.address};Database=${var.db_name};Username=${var.db_username};Password=${var.db_password}" }
    ]
    logConfiguration = {
      logDriver = "awslogs"
      options = {
...
# ─── ECS TASK + SERVICE: analytics-service ──────────────────────────
